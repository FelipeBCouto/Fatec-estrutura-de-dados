...
Deque (DOuble-End QUEue) é uma estrutura de dados derivada da fila que permite inserções e remoções em qualquer uma de suas extremidades. Com isso, o deque pode se comportar tanto como uma fila comum como quanto uma fila em que são admitidas inserções prioritárias e a possibilidade de sistência (remoção) do último item.
"""
...
class Deque:
    ...
    """
    Método que retorna se o deque está vazio (booleano)
    """
    def is_empty(self):
